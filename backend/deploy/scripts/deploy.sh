...
pre_deploy_checks() {
    ...
    command -v sam >/dev/null 2>&1 || { log_error "AWS SAM CLI is required but not installed."; exit 1; }
    command -v uv >/dev/null 2>&1 || { log_error "uv is required but not installed."; exit 1; }
...
health_check() {
    ...
    for i in {1..5}; do
        ...
        if curl -f "$HEALTH_URL" --max-time 30 --silent >/dev/null 2>&1; then
