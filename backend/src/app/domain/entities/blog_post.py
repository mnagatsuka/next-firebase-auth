...
class BlogPost:
    ...
    @classmethod
    def create_new(
        cls, title: str, content: str, excerpt: str, author: str, status: str = "draft"
    ) -> "BlogPost":
        """Factory method to create a new blog post."""
        post_id = str(uuid.uuid4())
        # Convert string status to PostStatus enum
        post_status = (
            PostStatus.PUBLISHED if status.lower() == "published" else PostStatus.DRAFT
        )

        # Set published_at if the post is being created as published
        published_at = (
            datetime.now(timezone.utc) if post_status == PostStatus.PUBLISHED else None
        )
